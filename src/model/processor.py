...
class Processor(ProcessorMixin):
    ...
    get words and normalized bounding polygons. These are then provided to [`LayoutLMv3Tokenizer`] or
    [`LayoutLMv3TokenizerFast`], which turns the words and bounding polygons into token-level `input_ids`,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        bounding polygons along with the additional arguments to [`~LayoutLMv3Tokenizer.__call__`] and returns the output,
        together with resized and normalized `pixel_values`. In case [`LayoutLMv3ImageProcessor`] was initialized with
        `apply_ocr` set to `False`, it passes the words (`text`/``text_pair`) and `polygons` specified by the user along
        ...
        """
        # verify if polygons are valid polygons
        if polygons is not None:
            for polygon in polygons:
                if len(polygon) != 8:
                    raise ValueError("Each box should contain 8 coordinates to represent a polygon.")

        # verify input
        if self.image_processor.apply_ocr and (polygons is not None):
            raise ValueError(
                "You cannot provide bounding polygons if you initialized the image processor with apply_ocr set to True."
            )
        ...
        encoded_inputs = self.tokenizer(
            ...
            polygons=polygons if polygons is not None else features["polygons"],
