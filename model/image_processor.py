...
import numpy as np
from transformers import is_vision_available, TensorType, requires_backends, LayoutLMv3ImageProcessor
from transformers.image_processing_utils import BaseImageProcessor, get_size_dict, BatchFeature
from transformers.image_transforms import to_pil_image, resize, to_channel_dimension_format
from transformers.image_utils import ChannelDimension, PILImageResampling, ImageInput, make_list_of_images, \
    valid_images, to_numpy_array, is_scaled_image, infer_channel_dimension_format
from transformers.utils import is_pytesseract_available, logging, IMAGENET_STANDARD_MEAN, IMAGENET_STANDARD_STD
...
def normalize_polygon(polygon, width, height):
    return [
        int(1000 * (polygon[0] / width)),
        int(1000 * (polygon[1] / height)),
        int(1000 * (polygon[2] / width)),
        int(1000 * (polygon[3] / height)),
        int(1000 * (polygon[4] / width)),
        int(1000 * (polygon[5] / height)),
        int(1000 * (polygon[6] / width)),
        int(1000 * (polygon[7] / height)),
    ]
...
def apply_tesseract(
    ...
    input_data_format: Optional[Union[str]] = None,
):
    """Applies Tesseract OCR on a document image, and returns recognized words + normalized bounding polygons."""

    # TODO Use DBNet or DBNet++ to get polygons and OCR instead of Tesseract, that only gets bboxes
    # TODO Right now this function returns polygon coordinates from a bbox, which is not really useful in skewed documents
    ...
    # turn coordinates into [xleft, ytop, xright, ytop, xright, ybottom, xleft, ybottom] format
    actual_polygons = []
    for x, y, w, h in zip(left, top, width, height):
        actual_polygon = [x, y, x + w, y, x + w, y + h, x, y + h]
        actual_polygons.append(actual_polygon)

    # finally, normalize the bounding polygons
    normalized_polygons = []
    for polygon in actual_polygons:
        normalized_polygons.append(normalize_polygon(polygon, image_width, image_height))

    assert len(words) == len(normalized_polygons), "Not as many words as there are bounding polygons"

    return words, normalized_polygons


class ImageProcessor(LayoutLMv3ImageProcessor):
    r"""
    ...
    Args:
        ...
        apply_ocr (`bool`, *optional*, defaults to `True`):
            Whether to apply the Tesseract OCR engine to get words + normalized bounding polygons. Can be overridden by
        ...
        **kwargs,
        ...
        Args:
            ...
            apply_ocr (`bool`, *optional*, defaults to `self.apply_ocr`):
                Whether to apply the Tesseract OCR engine to get words + normalized bounding polygons.
        ...
        # Tesseract OCR to get words + normalized bounding polygons
        if apply_ocr:
            requires_backends(self, "pytesseract")
            words_batch = []
            polygons_batch = []
            for image in images:
                words, polygons = apply_tesseract(image, ocr_lang, tesseract_config, input_data_format=input_data_format)
